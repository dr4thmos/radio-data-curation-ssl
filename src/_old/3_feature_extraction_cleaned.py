...
for batch_idx, batch in enumerate(dataloader):
    ...
    try:
        if args.model_type == "andrea_dino":
            raw_batch_features = extractor.extract_features(
                batches=[batch],
                module_name=args.module_name, 
                flatten_acts=False,             # <-- CHIAVE: Non appiattire, manteniamo la struttura spaziale/dei token
                output_type="ndarray",
            )
            print(f"raw_batch_features -> Shape: {raw_batch_features.shape}, Memory: {sys.getsizeof(raw_batch_features)} bytes")
            batch_features = raw_batch_features[:, 0, :]
            print(f"batch_features -> Shape: {batch_features.shape}, Memory: {sys.getsizeof(batch_features)} bytes")
        else:
        ...
        print(f"batch_paths memory: {sys.getsizeof(batch_paths)} bytes")
        # Store features and paths
        all_features.append(batch_features)
        all_image_paths.extend(batch_paths)
        print(f"all_features memory: {sys.getsizeof(all_features)} bytes")
        print(f"all_image_paths memory: {sys.getsizeof(all_image_paths)} bytes")
...
print(f"vstacked features memory: {sys.getsizeof(features)} bytes")
print(f"vstacked features shape: {features.shape}")
